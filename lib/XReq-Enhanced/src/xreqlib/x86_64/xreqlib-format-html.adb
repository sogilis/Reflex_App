...
package body XReqLib.Format.HTML is
   ...
   package Tmpl is new XReqLib.Format_HTML_Template
     (XReqLib.Standard_IO.T_Standard_IO,
      XReqLib.Standard_IO.Put);

   function To_String (N : Integer) return String;
   function Status_Class (Success : Status_Type) return String;
   function HTML_Text (S : String) return String;
   function HTML_Text (S : Unbounded_String) return String;
   procedure Put_Table (Format : in out HTML_Format_Type; T : Table_Type);
   ...
   function Status_Class (Success : Status_Type) return String is
   begin
      case Success is
         when Status_Passed =>
            return "pass";
   ...
   function HTML_Text (S : String) return String is
   ...
   function HTML_Text (S : Unbounded_String) return String is
   begin
      return HTML_Text (To_String (S));
   end HTML_Text;

   procedure Put_Table (Format : in out HTML_Format_Type; T : Table_Type) is
      use Table_Pkg;
      Cell    : Unbounded_String;
      Cell_Ok : Boolean;
   begin
      Tmpl.step_table_begin
        (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all));
      for Y in T.First_Y .. T.Last_Y loop
         Tmpl.step_table_row_begin
           (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all));
         for X in T.First_X .. T.Last_X loop
            T.Item (X, Y, Cell, Cell_Ok);
            if Cell_Ok then
               Tmpl.step_table_cell
                 (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all),
   ...
   is
   begin
      Tmpl.scenario_tags_begin
        (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all));
      for I in Tags'Range loop
         Tmpl.scenario_tags_item
           (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all),
            Param_tag => HTML_Text (To_String (Tags (I))));
      end loop;
      Tmpl.scenario_tags_end
        (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all));
   end Put_Tags;
   ...
   begin
      Tmpl.page_end (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all));
   end End_Tests;
   ...
   begin
      ...
      for I in 1 .. Integer (Length (Format.Menu)) loop
         ...
         if Length (E1.Sub_Menu) > 0 then
            ...
            Tmpl.report_menu_scenarios_end
              (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all));
         end if;
         Tmpl.report_menu_feature_end
           (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all));
      end loop;
      Tmpl.report_menu_end
        (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all));
      Tmpl.report_end (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all));
   end Put_Summary;
   ...
   procedure Put_Feature (Format : in out HTML_Format_Type) is
   begin
...
--        end if;
   ...
   procedure Enter_Outline (Format : in out HTML_Format_Type) is
   begin
      Tmpl.outline_begin
        (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all),
         Param_feature_id  => To_String (Format.Feature_ID),
   ...
   procedure Begin_Outline (Format : in out HTML_Format_Type) is
   begin
      Tmpl.outline_examples_begin
        (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all));
   end Begin_Outline;
   ...
   procedure Enter_Scenario (Format : in out HTML_Format_Type) is
   begin
   ...
   procedure Begin_Scenario (Format : in out HTML_Format_Type) is
   begin
      null;
   end Begin_Scenario;
   ...
   procedure Begin_Background (Format : in out HTML_Format_Type) is
   begin
   ...
   procedure End_Background (Format : in out HTML_Format_Type) is
   begin
      Tmpl.background_end
        (XReqLib.Standard_IO.T_Standard_IO (Format.Output.all));
   end End_Background;
...
--        Format.Output.Buffer_Commit;
   ...
   function New_And_Initialize
     (Text_IO_Interface : XReqLib.IO.T_Output_Class_Access)
      return not null HTML_Format_Ptr
   is
   begin
      return new HTML_Format_Type'(Output => Text_IO_Interface, others => <>);
