...
package body XReqLib.CLI is
   ...
   procedure Help (Name : String := Command_Name) is
   begin
      ...
      Put_Line ("");
      Put_Line ("    -c, --test-case TC");
      Put_Line ("        Choose a specific test_case to execute (only valid for scenario outline)");
      Put_Line ("");
   ...
   procedure Parse_Arguments
     (Args      : in out Argument_List_Access;
   ...
   is
      ...
      use XReqLib.Containers.String_Vectors;
      Parser  : Opt_Parser;
      Options : constant String :=
        "help h -help f: -format= o: -output= " &
        "-debug d -tags= t: -list -no-color -no-stacktrace c: -test-case=";
   ...
   begin
      ...
      while Getopt (Options, True, Parser) /= ASCII.NUL loop
         ...
         then
            declare
               N : constant String :=
                 Translate (Parameter (Parser), Lower_Case_Map);
               F : Format_Ptr := null;
            begin
               if N = "text" then
                  F :=
                    Format_Ptr
                      (XReqLib.Format.Text.New_And_Initialize
                         (XReqLib.IO.T_Output_Class_Access (IO_Interface)));
               elsif N = "html" then
                  F :=
                    Format_Ptr
                      (XReqLib.Format.HTML.New_And_Initialize
                         (XReqLib.IO.T_Output_Class_Access (IO_Interface)));
               else
                  F := null;
               end if;
         ...
         elsif Full_Switch (Parser) = "c" or
           Full_Switch (Parser) = "-test-case"
         then
            TC_Num := Integer'Value (To_String (To_Unbounded_String (Parameter (Parser))));
   ...
   exception
      ...
      when Invalid_Parameter =>
         Put_Line
           (Current_Error,
            "Missing parameter for switch -" & Full_Switch (Parser));
         Free (Parser);
         Free (Format);
         Help (Name);
         Success   := False;
   ...
   begin
      Get_Arguments (Args);
      Parse_Arguments (Args, Format, Continue, Cond, List_Mode, Succ, TC_Num, Name);
      if not Succ then
