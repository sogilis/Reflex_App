...
package body XReqLib.Format.Text is
   ...
   function ANSI (Sequence : String) return String renames XReqLib.ANSI.ANSI;
   function ANSI_G return String;
   function ANSI_R return String;
   function ANSI_C return String;
   function ANSI_X return String;

   function Color (Success : Status_Type) return String;

   -------------
   --  Color  --
   -------------

   function Color (Success : Status_Type) return String is
   begin
      case Success is
         when Status_Passed =>
            return ANSI_G;
...
--      return "";
   ...
   procedure Put_Feature (Format : in out Text_Format_Type) is
      ...
      Description : constant String := To_String (Format.Feature.Description);
   begin
   ...
   procedure End_Scenario (Format  : in out Text_Format_Type;
                           Success :        Status_Type) is
   begin
      if Success = Status_Passed then
         Append
           (Format.Passed_Step_List,
            "[ " & Format.Feature.Name & " ] " & Format.Scenario.Name & ASCII.LF);
      end if;
   end End_Scenario;
   ...
   procedure Put_Step
     (Format  : in out Text_Format_Type;
      ...
      Success :        Status_Type)
   is
      procedure Put_Long_String (Indent, Text : String);

      procedure Put_Long_String (Indent, Text : String) is
--         J : Natural := Text'First;
      begin
         Format.Output.Put_Line (Indent & """""""");
         Format.Output.Put (Indent);
         Format.Output.Put (Text);
...
--        end if;
      ...
      if Success = Status_Failed or
        (Success = Status_Skipped and Format.Step_ID = 1)
      then
         Append
           (Format.Failed_Step_List,
            "[ " & Format.Feature.Name & " ] " & Format.Scenario.Name & ASCII.LF);
--            "TODO COMMAND_NAME" --  Ada.Command_Line.Command_Name
--            & " " & Format.Scenario.Position & ASCII.LF);
      end if;

      if Format.Debug_Mode        --  Display every step in debug mode
        or
        not Format.In_Background  --  Display steps not in background
        or
        Format.Scenario_ID = 1    --  Display steps in first background
        or
        Success = Status_Failed   --  Display failed steps
        or
        Format.Background_Failed  --  Display steps after a failed step in a
      ...
      then
         ...
         then
            ...
            case Format.Step.Kind is
               when Step_Given =>
                  Format.Output.Put ("Given ");
               when Step_When =>
                  Format.Output.Put ("When ");
               when Step_Then =>
                  Format.Output.Put ("Then ");
            end case;
         end if;

         if XReqLib.ANSI.Use_ANSI_Sequences then
   ...
   begin
      Format.Output.New_Line;
      Format.Output.Put_Line ("    " & Format.S_Examples);
   ...
   procedure Put_Summary
     (Format : in out Text_Format_Type;
      ...
      D      :        Duration)
   is
      Count_Scenarios : constant Natural :=
        Report.Count_Scenario_Failed + Report.Count_Scenario_Passed;
...
--      Need_Comma  : Boolean;
   begin
      ...
      Format.Output.Put ("----");
      Format.Output.New_Line;
      Format.Output.Put ("Total Tests Run: " &
         Trim (Count_Scenarios'Img, Left));
      Format.Output.New_Line;
      Format.Output.New_Line;
--      if Count_Scenarios > 0 then
--         Format.Output.New_Line;
--      end if;
      Format.Output.Put ("Successful Tests: " &
         Trim (Report.Count_Scenario_Passed'Img, Left));
      Format.Output.New_Line;
      Format.Output.New_Line;
      if Format.Passed_Step_List /= Null_Unbounded_String then
         Format.Output.Put (To_String (Format.Passed_Step_List));
         Format.Output.New_Line;
      end if;
      Format.Output.Put ("Failed Tests: " &
         Trim (Report.Count_Scenario_Failed'Img, Left));
      Format.Output.New_Line;
      if Format.Failed_Step_List /= Null_Unbounded_String then
         Format.Output.Put (To_String (Format.Failed_Step_List));
         Format.Output.New_Line;
      end if;
--      Format.Output.Put (Trim (Count_Scenarios'Img, Left) & " scenario(s) (");
--      if Count_Scenarios > 1 then
--         Format.Output.Put (Trim (Count_Scenarios'Img, Left) & " scenario(s) (");
--      elsif Count_Scenarios = 1 then
--         Format.Output.Put (Trim (Count_Scenarios'Img, Left) & " scenario (");
--      else
--         Format.Output.Put (Trim (Count_Scenarios'Img, Left) & " scenarios");
--      end if;
...
--         Format.Output.Put (Trim (Count_Steps'Img, Left) & " steps (");
--      elsif Count_Steps = 1 then
--         Format.Output.Put (Trim (Count_Steps'Img, Left) & " step (");
--      else
--         Format.Output.Put (Trim (Count_Steps'Img, Left) & " steps");
--      end if;
      Format.Output.Put (Trim (Count_Scenarios'Img, Left) & " Scenario(s) (");
      ...
      Format.Output.New_Line;
      Format.Output.Put (Trim (Count_Steps'Img, Left) & " step(s) (");
      Format.Output.Put (
...
--         Format.Output.Put
--         (ANSI_R &
--          Trim (Report.Count_Steps_Failed'Img, Left) &
--          " failed" &
--          ANSI_X);
--         Need_Comma := True;
--      end if;
...
--         Format.Output.Put
--         (ANSI_C &
--          Trim (Report.Count_Steps_Skipped'Img, Left) &
--          " skipped" &
--          ANSI_X);
--         Need_Comma := True;
--      end if;
...
--         Format.Output.Put
--         (ANSI_G &
--          Trim (Report.Count_Steps_Passed'Img, Left) &
--          " passed" &
--          ANSI_X);
--         Need_Comma := True;
--      end if;
...
--      end if;
   ...
   is
   begin
      return new Text_Format_Type'(Output => Text_IO_Interface, others => <>);
