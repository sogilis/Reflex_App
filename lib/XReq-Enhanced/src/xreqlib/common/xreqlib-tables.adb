...
package body XReqLib.Tables is
   ...
   function Length (T : Table) return Key_Type is
   begin
   ...
   function Is_Empty (T : Table) return Boolean is
   begin
      return T.Count = 0;
   end Is_Empty;
   ...
   function First_X (T : Table) return Integer is
   begin
      return T.First_X;
   end First_X;
   ...
   function First_Y (T : Table) return Integer is
   begin
      return T.First_Y;
   end First_Y;
   ...
   function Last_X (T : Table) return Integer is
   begin
      return T.Last_X;
   end Last_X;
   ...
   function Last_Y (T : Table) return Integer is
   begin
      return T.Last_Y;
   end Last_Y;
   ...
   function Length_X (T : Table) return Integer is
   begin
   ...
   function Length_Y (T : Table) return Integer is
   begin
   ...
   function Item (T : Table; X, Y : Integer) return Element_Type is
      use Maps;
   begin
   ...
   procedure Put (T : in out Table; X, Y : Integer; Elem : Element_Type) is
      use Maps;
   begin
   ...
   function Has_Item (T : Table; X, Y : Integer) return Boolean is
      use Maps;
   begin
   ...
   procedure First_X (T : in out Table; X : Integer) is
   begin
   ...
   procedure First_Y (T : in out Table; Y : Integer) is
   begin
   ...
   procedure Last_X (T : in out Table; X : Integer) is
   begin
      T.Last_X := X;
      if T.Last_X < T.First_X then
         T.Last_X := T.First_X - 1;
   ...
   procedure Last_Y (T : in out Table; X : Integer) is
   begin
      T.Last_Y := X;
      if T.Last_Y < T.First_Y then
         T.Last_Y := T.First_Y - 1;
   ...
   function First (T : Table) return Cursor is
   begin
   ...
   function Key (C : Cursor) return Key_Type is
   begin
      return Maps.Key (C.C);
   end Key;
   ...
   function Header_Kind (T : Table) return Table_Header_Kind is
   begin
      return T.Head;
   end Header_Kind;
   ...
   function Data_Sets_Count (T : Table) return Natural is
   begin
      if T.Head = Transpose or T.Head = First_Column then
         return T.Length_Y;
      else
   ...
   function First_Data_Set (T : Table) return Table_Data_Set is
   ...
   function Last_Data_Set (T : Table) return Table_Data_Set is
   begin
   ...
   function Next_Data_Set (T : Table) return Table_Data_Set is
   begin
   ...
   function Data_Set_For (T : Table; H : Element_Type) return Table_Data_Set is
      D : Table_Data_Set := 1;
   begin
      loop
   ...
   function Records_Count (T : Table) return Natural is
      Length : Integer;
   begin
      if T.Head = Transpose or T.Head = First_Column then
         Length := T.Length_X;
   ...
   function Head_Record (T : Table) return Natural is
   begin
   ...
   function First_Record (T : Table) return Positive is
      pragma Unreferenced (T);
   begin
      return 1;
   end First_Record;
   ...
   function Last_Record (T : Table) return Natural is
   begin
   ...
   function Next_Record (T : Table) return Positive is
   begin
   ...
   function Is_Sparse (T : Table) return Boolean is
   begin
   ...
   is
      type Parsed_DS_Array is
        array
        (Table_Data_Set range
           Table_Data_Set'(1) ..
             Table_Data_Set (T.Data_Sets_Count)) of Boolean;
      D1               : Table_Data_Set  := 1;
      D2               : Table_Data_Set  := 1;
   ...
   function Transpose (T : Table) return Table is
      Res : Table;
      I   : Cursor;
      K   : Key_Type;
   begin
      ...
      case T.Header_Kind is
         when None =>
            Res.Set_Header_Kind (None);
         when First_Row =>
            Res.Set_Header_Kind (First_Column);
         when First_Column =>
            Res.Set_Header_Kind (First_Row);
         when Transpose =>
            Res.Set_Header_Kind (None);
      end case;
      return Res;
   ...
   function "=" (Left, Right : Table) return Boolean is
      use Maps;
   begin
      return Left.First_X = Right.First_X
        and then Left.First_Y = Right.First_Y
        and then Left.Last_X = Right.Last_X
        and then Left.Last_Y = Right.Last_Y
        and then Left.Count = Right.Count
        and then Left.Head = Right.Head
