...
package body XReq.Environment is
   ...
   procedure Make
     (Env      : in out Job_Environment;
      Step_Dir :        String_Vector := Empty_String_Vector;
      Out_Dir  :        String        := "";
      Language :        Language_Type := Lang_Ada)
   is
   begin
      Env :=
        (Reffy.Counted_Type (Env) with
         Step_Dir => Step_Dir,
   ...
   function First_Step_Dir (Env : Job_Environment) return String is
      use String_Vectors;
      use Ada.Containers;
   begin
   ...
   function Step_Dir (Env : Job_Environment) return String_Vector is
   begin
      return Env.Step_Dir;
   end Step_Dir;
   ...
   function Out_Dir (Env : Job_Environment) return String is
   begin
   ...
   begin
      ...
      if Is_Empty (Env.Step_Dir) then
         Append
           (Env.Step_Dir,
            To_Unbounded_String
              (Compose (Containing_Directory (Feature), "step_definitions")));
      end if;

      if Length (Env.Out_Dir) = 0 then
         Env.Out_Dir :=
           To_Unbounded_String
             (Compose (Containing_Directory (Feature), "tests"));
      end if;
   ...
   function Language (Env : Job_Environment) return Language_Type is
   begin
      return Env.Language;
   end Language;
   ...
   function Loaded (Env : Job_Environment) return Boolean is
   begin
      return Env.Loaded;
   end Loaded;
   ...
   function Steps (Env : Job_Environment) return Step_File_List_Handle is
   begin
      return Env.Steps;
   end Steps;
