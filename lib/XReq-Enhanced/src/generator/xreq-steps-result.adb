...
package body XReq.Steps.Result is
   ...
   begin
      ...
      Self          := Result_Step_Type'(Step_Type (Self) with Match => Match);
   end Make;
   ...
   begin
      ...
      if not Match.Match then
         ...
         if Log.Verbosity < 0 then
            ...
            Log.Put_Line
            (String'
               ("ERROR: Missing step definition in " &
                To_String (Stanza.R.Position) &
                " for:"));
            Log.Put_Line ("  " & Stanza.R.To_String);
            Log.Put_Line
            ("You can implement this step by adding on your " &
   ...
   function File_Name (S : Result_Step_Type) return String is
   begin
   ...
   function Procedure_Name (S : Result_Step_Type) return String is
   begin
   ...
   function Match_First (S : Result_Step_Type) return Natural is
   ...
   function Match_Last (S : Result_Step_Type) return Integer is
   begin
   ...
   function Match_Count (S : Result_Step_Type) return Natural is
