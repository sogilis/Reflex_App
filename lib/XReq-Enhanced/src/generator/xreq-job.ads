...
package XReq.Job is
   ...
   function Feature_File (Job : Job_Type) return String;
   function Result (Job : Job_Type) return Result_Feature_Handle;

   procedure Make (Job : out Job_Type; Feature_File : String);
   procedure Run
     (Job               : in out Job_Type;
   ...
   --  package
   --  If Syntax_Check_Only is True, only syntaxt check is done on feature file
   --  No step_definition generation...
...
private
   ...
   type Job_Type is tagged record
