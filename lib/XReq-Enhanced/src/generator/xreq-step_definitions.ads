...
package XReq.Step_Definitions is
   ...
   type Step_Match_Type is record
      Match     : Boolean := False;
   ...
   type Find_Result_Procedure is access procedure (Match : Step_Match_Type);
   ...
   function File_Name (S : Step_File_Type) return String;
   function Parsed (S : Step_File_Type) return Boolean;
   procedure Parse
     (S      : in out Step_File_Type;
      Logger :        Logger_Ptr) is abstract;
...
private
   ...
   type Step_Definition_Type is new Ada.Finalization.Controlled with record
      Prefix    : Step_Kind;
   ...
   package Step_Container is new Ada.Containers.Vectors
     (Natural,
      Step_Definition_Type);

   type Step_File_Type is abstract new Reffy.Counted_Type with record
