...
package body Util.IO is
   ...
   procedure Add_Verbosity (Log : in out Logger_Type; V : Integer) is
   begin
      Log.Verbosity_Level := Log.Verbosity_Level + V;
   end Add_Verbosity;

   function Verbosity (Log : Logger_Type) return Integer is
   begin
   ...
   procedure Put (Log : in out Logger_Type; V : Integer; S : String) is
   ...
   procedure Put_Indent (Log : in out Logger_Type; V : Integer := 0) is
   begin
      Log.Put (V, String'(Log.Indent_Level * " "));
   end Put_Indent;

   procedure New_Line (Log : in out Logger_Type; V : Integer := 0) is
   begin
      Log.Put (V, "" & ASCII.LF);
   end New_Line;

   procedure Indent (Log : in out Logger_Type; N : Natural := 3) is
   begin
      Log.Indent_Level := Log.Indent_Level + N;
   end Indent;

   procedure UnIndent (Log : in out Logger_Type; N : Natural := 3) is
   begin
      Log.Indent_Level := Log.Indent_Level - N;
   end UnIndent;
   ...
   procedure Put_Always (Log : in out Standard_Logger_Type; S : String) is
   ...
   procedure Put_Always (Log : in out Buffer_Logger_Type; S : String) is
   begin
      Append (Log.Buffer, S);
   end Put_Always;
   ...
   function Get_File (File_Name : String) return String is
      use Char_IO;
      File : Char_IO.File_Type;
   ...
   procedure Set_File (File_Name : String; Content : String) is
      use Char_IO;
      File : Char_IO.File_Type;
   begin
   ...
   procedure Append_File (File_Name : String; Content : String) is
      use Char_IO;
      File : Char_IO.File_Type;
   begin
   ...
   function Get_Whole_Line (File : Ada.Text_IO.File_Type) return String is
--      use Ada.Text_IO;
   begin
   ...
   function GetEnv (VarName, Default : String) return String is
   ...
   function To_String (Pos : Position_Type) return String is
   begin
      return To_String (Pos.File) & ":" & Trim (Pos.Line'Img, Left);
   end To_String;

   function Position (File : String; Line : Natural) return Position_Type is
   begin
