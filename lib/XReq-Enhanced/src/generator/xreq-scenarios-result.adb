...
package body XReq.Scenarios.Result is
   ...
   procedure Process_Scenario
     (Res           : in out Result_Scenario_Type;
      Scenario      :        Scenario_Handle;
      Steps         :        Step_File_List_Handle;
      Log           :        Logger_Ptr;
      Errors        :    out Boolean;
      Missing_Steps : in out String_Set;
      Step_Matching :        Boolean := False)
   is
   ...
   begin
      ...
      for I in Scenario.R.Step_First .. Scenario.R.Step_Last loop
         ...
         if Scenario.R.Outline then
            ...
            (Scenario.R.Step_Element
             (I), Steps, Log, Err, Step_Matching, Missing_Steps);
         end if;
         if Err then
            Errors := True;
         else
      ...
      if Scenario.R.Outline then
         ...
         for Y in Scenario.R.Table.First_Y + 1 .. Scenario.R.Table.Last_Y loop
            ...
            for X in Scenario.R.Table.First_X .. Scenario.R.Table.Last_X loop
               declare
                  ...
                  Label : constant String :=
                    "<" &
                    Scenario.R.Table.Item (X, Scenario.R.Table.First_Y, "") &
                    ">";
               begin
                  K := First (Steps_tmp2);
                  while Has_Element (K) loop
                     ...
                     St.R.Set_Stanza (Replace (St.R.Stanza, Label, Item));
            ...
            while Has_Element (K) loop
               ...
               Res_St := Create;
               Res_St.R.Process_Step
               (St, Steps, Log, Err, Step_Matching, Missing_Steps);
               if Err then
                  Errors := True;
               end if;
...
--      use Result_Steps;
   ...
   function Outline_First (S : Result_Scenario_Type) return Natural is
   ...
   function Outline_Last (S : Result_Scenario_Type) return Integer is
   begin
   ...
   function Outline_Count (S : Result_Scenario_Type) return Natural is
      use Result_Steps_Vectors2;
      use Ada.Containers;
   begin
   ...
   function Outline_Step_First
     (S       : Result_Scenario_Type;
      Outline : Natural) return Natural
   is
   ...
   function Outline_Step_Last
     (S       : Result_Scenario_Type;
      Outline : Natural) return Integer
   is
   begin
   ...
   function Outline_Step_Element
     (S       : Result_Scenario_Type;
      Outline : Natural;
      Step    : Natural) return Result_Step_Handle
   is
      use Result_Steps;
      use Result_Steps_Vectors2;
   ...
   procedure Step_Append
     (Scenario : in out Result_Scenario_Type;
      Stanza   :        Result_Step_Handle)
   is
   ...
   function Step_Element
     (Scenario : Result_Scenario_Type;
      Index    : Natural) return Result_Step_Handle
   is
