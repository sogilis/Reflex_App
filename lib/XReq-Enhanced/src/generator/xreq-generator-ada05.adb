...
package body XReq.Generator.Ada05 is
   ...
   begin
      ...
      Get_Unique_String (Gen.Pool, To_Identifier (Pkgname), Gen.Id_Pkgname);
      Get_Unique_String
        (Gen.Pool,
         To_Identifier ("Background"),
         Gen.Fn_Backgnd);
      Basename :=
        To_Unbounded_String
          (Compose (Env.Ref.Out_Dir, To_Lower (To_String (Gen.Id_Pkgname))));
...
--      use Match_Vectors;
      ...
      E2       : Argument_Type;
      --
      Procname_Without_PkgName : Unbounded_String;
   begin
      S.Adb.Put_Line ("--");
      ...
      S.Adb.New_Line;
      S.Adb.Put_Line
      ("Stanza : constant String    := " & Ada_String (Step.R.Stanza) & ";");
      S.Adb.Put_Line
      ("Pos    : constant String    := " &
      ...
      --  Fill in Args  --
      -------------------------------------------------------------------------
      S.Adb.Put_Line ("Make (Args, " & Ada_String (Step.R.Stanza) & ");");
      for I in Step.R.Match_First .. Step.R.Match_Last loop
         E := Step.R.Match_Element (I);
         S.Adb.Put_Line
         ("Add_Match (Args," & E.First'Img & "," & E.Last'Img & ");");
      end loop;
      for I2 in Step.R.Arg_First .. Step.R.Arg_Last loop
         ...
         case E2.Typ is
            when Text =>
               S.Adb.Put_Line
               ("Add_Text  (Args, " & Ada_String (To_String (E2.Text)) & ");");
            when Table =>
               S.Adb.Put_Line ("declare");
               S.Adb.Indent;
      ...
      if Fake then
         ...
         S.Adb.Put_Line
         ("   with ""The step definition cound not be " & "found"";");
      else
         ...
         for K in reverse Procname'Range loop
            if Copy then
               ...
               Copy := True;
               Procname_Without_PkgName := To_Unbounded_String (Procname (K + 1 .. Procname'Last));
            end if;
         end loop;
         ...
         S.Adb.Put_Line (Procname_Without_PkgName & " (Args);");
   ...
   procedure Generate_Tag_Variable
     (S        : in out Ada_Generator_Type;
      Scenario : in     Result_Scenario_Handle)
   is
   ...
   begin
      S.Adb.Put_Line
      ("Tags : constant " & Generate_Tag_List_Type (Scenario) & " :=");
      S.Adb.Put_Indent;
      S.Adb.Put ("      ");
...
--      use String_Vectors;
   ...
   begin
      ...
      --  Report     : Counters of passed, skipped and failed steps/scenarios
...
--      S.Adb.Put_Line (Proc & "(Format     : in out Format_Ptr;");
      S.Ads.Put_Line (Proc & "(Format     : in     Format_Ptr;");
      S.Adb.Put_Line (Proc & "(Format     : in     Format_Ptr;");
      S.Ads.Put_Indent;
...
--      Generate_Tag_Variable (S, Scenario);
      ...
      S.Adb.Put_Line
      ("   Report.Num_Steps := Report.Num_Steps +" &
       Scenario.R.Step_Count'Img &
       ";");
      S.Adb.Put_Line ("else");
      S.Adb.Indent;
      S.Adb.Put_Line
      ("Format.Start_Background (" &
   ...
   procedure Generate_Scenario_Body
     (S                  : in out Ada_Generator_Type;
      Scenario           : in     Result_Scenario_Handle;
      TC_Name            : in     String;
      Current_Outline_ID : in     Integer := -1;
      Last_Outline_ID    : in     Integer := -1)
   is
      use String_Sets;

      Proc     : constant String := "procedure " & TC_Name & " ";
      Pkgname  : Unbounded_String;
      Copy     : Boolean := False;
   begin
      S.Adb.New_Line;
      ...
      S.Adb.Put_Indent;
      S.Adb.Put (Proc'Length * " ");
      S.Adb.Put (" Tags       : in XReqLib.Format.Tag_Array_Type;");
      S.Adb.New_Line;
      S.Adb.Put_Indent;
      S.Adb.Put (Proc'Length * " ");
      S.Adb.Put (" Fail       : in out Boolean;");
      S.Adb.New_Line;
      S.Adb.Put_Indent;
      S.Adb.Put (Proc'Length * " ");
      S.Adb.Put (" Count_Mode : in     Boolean := False)");
      ...
      S.Adb.Put_Line ("if Count_Mode then");
      S.Adb.Indent;
      S.Adb.Put_Line (S.Fn_Backgnd & " (Format, Report, Stop, True);");
      S.Adb.Put_Line
        ("Report.Num_Steps := Report.Num_Steps +" &
           Scenario.R.Step_Count'Img &
           ";");
      ...
      --  Scenario Start
      -------------------
      if Current_Outline_ID < 0 then
         S.Adb.Put_Line
           ("Format.Start_Scenario (" &
              Ada_String (Scenario.R.Name) &
              ", " &
              Ada_String (To_String (Scenario.R.Position)) &
              ", " &
         ...
         S.Adb.Put_Line
           ("Format.Start_Scenario (" &
              Ada_String (Scenario.R.Name &
                " (" & To_String(Trim(To_Unbounded_String(Integer'Image (Current_Outline_ID + 1)), Ada.Strings.Left)) &
                "/" & To_String(Trim(To_Unbounded_String(Integer'Image (Last_Outline_ID + 1)), Ada.Strings.Left)) &
                ")") &
              ", " &
              Ada_String (To_String (Scenario.R.Position)) &
              ", " &
      ...
      if Current_Outline_ID < 0 then
         declare
         Procname : constant String := Scenario.R.all.Step_Element (Scenario.R.Step_First).R.Procedure_Name;
         begin
            for K in reverse Procname'Range loop
               if Copy then
                  Pkgname := Procname (K) & Pkgname;
         ...
         declare
            Procname : constant String := Scenario.R.all.Outline_Step_Element (Current_Outline_ID, Scenario.R.Outline_Step_First (Current_Outline_ID)).R.Procedure_Name;
         begin
            for K in reverse Procname'Range loop
               if Copy then
                  Pkgname := Procname (K) & Pkgname;
      ...
      S.Adb.New_Line;
      S.Adb.Put_Line ("--------------");
      S.Adb.Put_Line ("--  Set_Up  --");
      S.Adb.Put_Line ("--------------");
      S.Adb.New_Line;
      S.Adb.Put_Line ("Set_Up (" & Ada_String (To_String (Scenario.R.Position.File)) & ");");
      S.Adb.Put_Line ("Fail := Stop;");
      ...
      S.Adb.Put_Line ("------------------");
      S.Adb.New_Line;
      S.Adb.Put_Line (S.Fn_Backgnd & " (Format, Report, Stop, Count_Mode);");
      S.Adb.Put_Line ("Fail := Stop;");
      ...
      if Current_Outline_ID < 0 then
         ...
         for I in
           Scenario.R.Outline_Step_First (Current_Outline_ID) ..
               Scenario.R.Outline_Step_Last (Current_Outline_ID)
         loop
            Generate_Step
              (S,
               Scenario,
               Scenario.R.all.Outline_Step_Element (Current_Outline_ID, I));
         end loop;
      end if;
      ...
      S.Adb.Put_Line ("if Fail then");
      S.Adb.Put_Line
        ("   Report.Count_Scenario_Failed := " &
           "Report.Count_Scenario_Failed + 1;");
      S.Adb.Put_Line ("Format.End_Scenario (Status_Failed);");
      S.Adb.Put_Line ("else");
      S.Adb.Put_Line
        ("   Report.Count_Scenario_Passed := " &
           "Report.Count_Scenario_Passed + 1;");
      S.Adb.Put_Line ("Format.End_Scenario (Status_Passed);");
      S.Adb.Put_Line ("end if;");

      S.Adb.New_Line;
      S.Adb.Put_Line ("-----------------");
      S.Adb.Put_Line ("--  Tear_Down  --");
      S.Adb.Put_Line ("-----------------");
      S.Adb.New_Line;
      S.Adb.Put_Line ("Tear_Down (" & Ada_String (To_String (Scenario.R.Position.File)) & ");");
      S.Adb.Put_Line ("Fail := Stop;");
...
--      use String_Vectors;
      ...
      Proc_Test_Case : Unbounded_String;
   begin
      pragma Assert (Scenario.Valid);
      ...
      -- First, generate single procedure for each test case
      if not Scenario.R.Outline then
         Test_Case_Name := Name & "_TC";
         Proc_Test_Case := "procedure " & Test_Case_Name & " ";
         Generate_Scenario_Body (S => S,
                                 Scenario => Scenario,
                                 TC_Name => To_String (Test_Case_Name));
      else
         for I in Scenario.R.Outline_First .. Scenario.R.Outline_Last loop
            Test_Case_Name := Name & "_TC_" & Trim(To_Unbounded_String(Integer'Image (I + 1)), Ada.Strings.Left);
            Proc_Test_Case := "procedure " & Test_Case_Name & " ";
            Generate_Scenario_Body (S => S,
                                    Scenario => Scenario,
                                    TC_Name => To_String (Test_Case_Name),
                                    Current_Outline_ID => I,
                                    Last_Outline_ID => Scenario.R.Outline_Last);
         end loop;
      end if;
...
--      S.Adb.Put_Line (Proc & "(Format     : in out Format_Ptr;");
      ...
      S.Adb.Put_Indent;
      S.Adb.Put (Proc'Length * " ");
      S.Ads.Put (" TC_Num     : in     Integer := -1);");
      S.Adb.Put (" TC_Num     : in     Integer := -1)");
      S.Ads.New_Line;
      S.Adb.New_Line;
      S.Ads.New_Line;
      S.Adb.New_Line;
      ...
      S.Adb.Indent;
      if not Scenario.R.Outline then
         S.Adb.Put_Line ("pragma Unreferenced (TC_Num);");
      end if;
--        if Scenario.R.Outline then
--          S.Adb.Put_Line ("Outline_Table : Table_Type;");
--        end if;
      S.Adb.Put_Line
        ("Num_Scenario  : constant Natural :=" & Seq_Num'Img & ";");
      S.Adb.Put_Line ("Fail : Boolean := Stop;");
      Generate_Tag_Variable (S, Scenario);
      S.Adb.New_Line;
      S.Adb.UnIndent;
      ...
      S.Adb.Put_Line
      ("if Cond.Eval (Tags) and then Cond.Eval (" &
      ...
      if not Scenario.R.Outline then
         Proc_Test_Case := Name & "_TC" & " ";
         S.Adb.Put_Line ("--------------------");
         S.Adb.Put_Line ("--  Test Case     --");
         S.Adb.Put_Line ("--------------------");
         S.Adb.Put_Line (Proc_Test_Case & "(Format     => Format,");
         S.Adb.Put_Indent;
         S.Adb.Put (To_String(Proc_Test_Case)'Length * " ");
         S.Adb.Put (" Report     => Report,");
         S.Adb.New_Line;
         S.Adb.Put_Indent;
         S.Adb.Put (To_String(Proc_Test_Case)'Length * " ");
         S.Adb.Put (" Stop       => Stop,");
         ...
         S.Adb.Put_Indent;
         S.Adb.Put (To_String(Proc_Test_Case)'Length * " ");
         S.Adb.Put (" Count_Mode => Count_Mode);");
         S.Adb.New_Line;
      else
         for I in Scenario.R.Outline_First .. Scenario.R.Outline_Last loop
            Proc_Test_Case := Name & "_TC_" & Trim(To_Unbounded_String(Integer'Image (I + 1)), Ada.Strings.Left) & " ";
            S.Adb.Put_Line ("--------------------");
            S.Adb.Put_Line ("--  Test Case " & Trim(To_Unbounded_String(Integer'Image (I + 1)), Ada.Strings.Left));
            S.Adb.Put_Line ("--------------------");
            S.Adb.Put_Line ("if TC_Num = -1 or TC_Num = " & Trim(To_Unbounded_String(Integer'Image (I + 1)), Ada.Strings.Left) & " then");
            S.Adb.Indent;
...
--           S.Adb.Put_Line ("end if;");
   ...
   procedure Generate_Feature (S : in out Ada_Generator_Type) is
      use String_Vectors;
   ...
   begin
      ...
      for I in S.Feature.R.Scenario_First .. S.Feature.R.Scenario_Last loop
         ...
         Get_Unique_String
           (S.Pool,
            To_Identifier ("Scenario_" & Sce.R.Name),
            Str);
   ...
   procedure Generate_With (S : in out Ada_Generator_Type) is
      use String_Sets;
      J   : String_Sets.Cursor := First (S.With_Pkg);
      Buf : Unbounded_String;
   begin
      while Has_Element (J) loop
         Append (Buf, "with " & Element (J) & ";" & " use " & Element (J) & ";" & S.Adb.CRLF);
         Next (J);
      end loop;
   ...
   procedure Generate (Gen : in out Ada_Generator_Type; Log : in Logger_Ptr) is
      use String_Vectors;
      use Ada.Containers;
   ...
   begin
      ...
      Gen.Adb.Put_Line ("with Ada.Strings.Unbounded;");
      Gen.Ads.Put_Line ("with XReqLib;");
      Gen.Adb.Put_Line ("with XReqLib.Args;");
      ...
      Gen.Adb.Put_Line ("use  XReqLib.Args;");
      ...
      Gen.Adb.New_Line;
      Gen.Adb.Put_Line
        ("Str_Feature    : constant String := " &
           Ada_String (Lang.R.Feature) &
           ";");
--      Gen.Adb.Put_Line
--      ("Str_Background : constant String := " &
--       Ada_String (Lang.R.Background) &
--       ";");
      Gen.Adb.Put_Line
        ("Str_Scenario   : constant String := " &
           Ada_String (Lang.R.Scenario) &
           ";");
      Gen.Adb.Put_Line
        ("Str_Outline    : constant String := " &
           Ada_String (Lang.R.Scenario_Outline) &
           ";");
      Gen.Adb.Put_Line
        ("Str_Examples   : constant String := " &
           Ada_String (Lang.R.Examples) &
           ";");
--      Gen.Adb.Put_Line
--      ("Str_Given      : constant String := " &
--       Ada_String (Lang.R.Given) &
--       ";");
--      Gen.Adb.Put_Line
--      ("Str_When       : constant String := " &
--       Ada_String (Lang.R.When_K) &
--       ";");
--      Gen.Adb.Put_Line
--      ("Str_Then       : constant String := " &
--       Ada_String (Lang.R.Then_K) &
--       ";");
--      Gen.Adb.Put_Line
--      ("Str_And        : constant String := " &
...
--      Gen.Ads.Put_Line ("procedure Run (Format     : in out Format_Ptr;");
      Gen.Ads.Put_Line ("procedure Run (Format     :        Format_Ptr;");
      Gen.Ads.Put_Line ("               Cond       :        Conditional_Type;");
      Gen.Ads.Put_Line ("               Report     : in out Report_Type;");
      Gen.Ads.Put_Line ("               List_Mode  :        Boolean := False;");
      Gen.Ads.Put_Line ("               Count_Mode :        Boolean := False;");
      Gen.Ads.Put_Line ("               TC_Num     :        Integer := -1);");
--      Gen.Adb.Put_Line ("procedure Run (Format     : in out Format_Ptr;");
      Gen.Adb.Put_Line ("procedure Run (Format     :        Format_Ptr;");
      Gen.Adb.Put_Line ("               Cond       :        Conditional_Type;");
      Gen.Adb.Put_Line ("               Report     : in out Report_Type;");
      Gen.Adb.Put_Line ("               List_Mode  :        Boolean := False;");
      Gen.Adb.Put_Line ("               Count_Mode :        Boolean := False;");
      Gen.Adb.Put_Line ("               TC_Num     :        Integer := -1) is");
...
--      Gen.Adb.Put_Line ("First : Boolean  := True;");
      ...
      Gen.Adb.Put_Line ("Format.S_Examples (Str_Examples);");
      ...
      for I in 0 .. Integer (Length (Gen.Fn_Steps)) - 1 loop
         Gen.Adb.Put_Line
           (Element (Gen.Fn_Steps, I) &
              " " &
              "(Format, Report, Cond, Stop, Count_Mode, TC_Num);");
      end loop;
   ...
   procedure Generate_Suite
     (Gens : in Generator_Vectors.Vector;
      Name : in String;
      Env  : in Environment_Handle;
      Log  : in Logger_Ptr;
      Pltf : in Target_Type;
      TXT_IO : in Output_IO_Type;
      Cov  : in Boolean := False;
...
--        With_B.Put_Line ("with XReqLib;");
      ...
      if Pltf = Target_Arm_Eabi then
         With_B.Put_Line ("with XReqLib.Format.Text;");
         With_B.Put_Line ("with XReqLib.Format.Multi;");
         With_B.Put_Line ("with XReqLib.IO;");
         if TXT_IO = Output_IO_Std then
            With_B.Put_Line ("with XReqLib.Standard_IO;");
         elsif TXT_IO = Output_IO_Serial then
            With_B.Put_Line ("with XReqLib.Serial_IO;");
         end if;
         With_B.Put_Line ("with XReqLib.ANSI;");
      end if;
...
--         With_B.Put_Line ("use  XReqLib.Format.Text;");
         With_B.Put_Line ("use  XReqLib.Format.Multi;");
      end if;
      With_B.New_Line;
      ...
      if Pltf = Target_x86_64 then
         Body_B.Put_Line
         ("Self_Name : constant String := " & Ada_String (Prc_Name) & ";");
      end if;
      if Pltf = Target_x86_64 then
         Body_B.Put_Line ("Continue   : Boolean := True;");
      elsif Pltf = Target_Arm_Eabi then
         Body_B.Put_Line ("Continue   : constant Boolean := True;");
      end if;
      ...
      if Pltf = Target_Arm_Eabi then
         ...
         if TXT_IO = Output_IO_Std then
            Body_B.Put_Line
            ("IO_Interface : XReqLib.Standard_IO.T_Standard_IO_Class_Access := null;");
         elsif TXT_IO = Output_IO_Serial then
            Body_B.Put_Line
            ("IO_Interface : XReqLib.Serial_IO.T_Serial_IO_Class_Access := null;");
         end if;
         Body_B.Put_Line ("F            : Format_Ptr := null;");
      end if;
      Body_B.Put_Line ("Format     : Format_Ptr := null;");
      if Pltf = Target_x86_64 then
         Body_B.Put_Line ("Cond       : Conditional_Type := Null_Condition;");
         Body_B.Put_Line ("Test_Case_Num : Integer := -1;");
      elsif Pltf = Target_Arm_Eabi then
         Body_B.Put_Line ("Cond       : constant Conditional_Type := Null_Condition;");
         Body_B.Put_Line ("Test_Case_Num : constant Integer := -1;");
      end if;
      if Pltf = Target_x86_64 then
         Body_B.Put_Line ("List_Mode  : Boolean := False;");
      elsif Pltf = Target_Arm_Eabi then
         Body_B.Put_Line ("List_Mode  : constant Boolean := False;");
      end if;
      ...
      if Pltf = Target_x86_64 then
         Body_B.Put_Line
         ("Parse_Arguments (Format, Continue, Cond, " &
          "List_Mode, Test_Case_Num, Self_Name);");
      elsif Pltf = Target_Arm_Eabi then
         Body_B.Put_Line
           ("XReqLib.ANSI.Use_ANSI_Sequences := False;");
         if TXT_IO = Output_IO_Std then
            Body_B.Put_Line
            ("IO_Interface := new XReqLib.Standard_IO.T_Standard_IO;");
         elsif TXT_IO = Output_IO_Serial then
            Body_B.Put_Line
            ("IO_Interface := new XReqLib.Serial_IO.T_Serial_IO;");
            Body_B.Put_Line
            ("IO_Interface.Init;");
         end if;
         Body_B.Put_Line
           ("F  := Format_Ptr (XReqLib.Format.Text.New_And_Initialize(" &
              "XReqLib.IO.T_Output_Class_Access (IO_Interface)));");
         Body_B.Put_Line ("Multi_Format.Add_Sub_Format (F);");
         Body_B.Put_Line ("Format := Format_Ptr (Multi_Format);");
      end if;
      ...
      Body_B.Put_Line ("Call_Features (Format, Cond, Report, List_Mode, TC_Num => Test_Case_Num);");
...
--      Body_B.Put_Line    ("end if;");
      ...
      if Cov then
         Gpr_B.Put_Line ("package Linker is");
         Gpr_B.Indent;
         Gpr_B.Put_Line ("for Default_Switches (""Ada"") use (""-fprofile-arcs"",""-fprofile-generate"");");
         Gpr_B.UnIndent;
         Gpr_B.Put_Line ("end Linker;");
      end if;
      ...
      begin
         ...
         while More_Entries (Results) loop
            ...
            while not Generated and then Has_Element (I) loop
               E         := Ada_Generator_Ptr (Element (I));
               Generated :=
                 Simple_Name (To_String (E.Ads_File)) =
            ...
            if not Generated then
               declare
                  File : constant String :=
                    Compose (Env.Ref.Out_Dir, Simple_Name (Dir_Entry));
