...
package body XReq.Features.Files is
   ...
   procedure Make (F : out Feature_File_Type; File_Name : String) is
   ...
   function Create (File_Name : String) return Feature_File_Type is
      F : Feature_File_Type;
   begin
      Make (F, File_Name);
      return F;
   ...
   procedure Parse (F : in out Feature_File_Type; Log : Logger_Ptr) is
      ...
      procedure Log_Error (Error : String);
      procedure Read_Line;
      function End_Of_File return Boolean;
      function Detect_Keyword (Keyword : String) return Boolean;

      procedure Read_All;
      procedure Read_Keywords;
      procedure Read_Feature (Feature : in out Feature_File_Type);
      procedure Read_Scenario
        (Scenario : out Scenario_Handle;
         Outline  :     Boolean := False);
      procedure Read_Step (Step : in out Step_Handle);
      procedure Read_String (Result : out Unbounded_String; Sep : String);
      procedure Read_Table (Result : out XReqLib.String_Tables.Table);
      ...
      procedure Log_Error (Error : String) is
--         use Ada.Strings;
--         use Ada.Strings.Fixed;
      ...
      function Detect_Keyword (Keyword : String) return Boolean is
         Detect : Boolean;
      begin
      ...
      begin
         ...
         while Continue and not End_Of_File loop
            ...
            if Detect_Keyword (K.Background)
              or else Detect_Keyword (K.Scenario)
              or else Detect_Keyword (K.Scenario_Outline)
              or else Detect_Keyword ("@")
            then
               Unread_Line := True;
               Continue    := False;
               Detect      := False;
               ...
               if Current_Prefix = Step_Null then
                  if Log.Verbosity < 0 then
                     Log_Error
                       ("ERROR: And keyword should be following " &
                        "another keyword");
                     Log_Error ("ERROR: Ignoring step");
                  else
                     Log_Error ("ERROR: And keyword");
                     Log.Put_Line
                     ("       And keyword should be following " &
                      "another keyword");
      ...
      procedure Read_Step (Step : in out Step_Handle) is
         ...
         Tble        : XReqLib.String_Tables.Table;
      begin
         Step.R.Make
         (Step.R.Kind, To_String (Trimed_Suffix (Line_S, Idx_Data)), Position);
--          Log_Error ("Begin read step");
         while Continue and not End_Of_File loop
            ...
            if Detect_Keyword (K.Background)
              or else Detect_Keyword (K.Scenario)
              or else Detect_Keyword (K.Scenario_Outline)
              or else Detect_Keyword (K.Examples)
              or else Detect_Keyword (K.Given)
              or else Detect_Keyword (K.When_K)
              or else Detect_Keyword (K.Then_K)
              or else Detect_Keyword (K.And_K)
              or else Detect_Keyword ("@")
            then
               Unread_Line := True;
               Continue    := False;
...
--          Log_Error ("End   read step");
      ...
      --  ++                | ''' NL { char | \' } NL { space } ''' NL
      procedure Read_String (Result : out Unbounded_String; Sep : String) is
      ...
      begin
         ...
         while Continue and not End_Of_File loop
            ...
            if Detect_Keyword (Sep)
              and then Trimed_Suffix (Line_S, Idx_Data) = ""
            then
               ...
               begin
                  ...
                  while I <= Line'Last loop
                     if I <= Line'Last - 1
                       and then Line (I .. I + 1) = "\" & Sep (Sep'First)
                     then
...
--                                    Trim (Line (I .. N - 1), Both));
   ...
   begin
      ...
      Self.Lang.Set_New (K);
      Position := Position_Type'(File => Self.File_Name, Line => 1);
   ...
   function File_Name (F : Feature_File_Type) return String is
   begin
   ...
   function To_String (F : Feature_File_Type) return String is
   ...
   function Parsed (F : Feature_File_Type) return Boolean is
   begin
      return F.Parsed;
   end Parsed;
