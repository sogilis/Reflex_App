...
package body XReq.Generator.C is
   ...
   procedure Generate_Table
     (S    : in out C_Generator_Type;
      Name :        String;
      T    :        String_Tables.Table);
   procedure Generate_Step
     (S          : in out C_Generator_Type;
      Scenario   :        Result_Scenario_Handle;
      Step       :        Result_Step_Handle;
      Num        :        Natural;
      Background :        Boolean := False;
      Fake       :        Boolean := False;
      Outline    :        Boolean := False;
      Num_Outlne :        Natural := 1);
   ...
   begin
      ...
      Get_Unique_String (Gen.Pool, To_Identifier (Pkgname), Gen.Header_Name);
      Get_Unique_String
        (Gen.Pool,
      ...
      Basename :=
        To_Unbounded_String
          (Compose (Env.Ref.Out_Dir, To_Lower (To_String (Gen.Header_Name))));
...
--      use Match_Vectors;
   ...
   begin
      ...
      case Step.R.Kind is
         when Step_Given =>
            S.C.Put ("XReq_Kind_Given");
         when Step_When =>
            S.C.Put ("XReq_Kind_When");
         when Step_Then =>
            S.C.Put ("XReq_Kind_Then");
      end case;
      S.C.New_Line;
      S.C.Put_Line ("#define stanza " & C_String (Step.R.Stanza));
      S.C.Put_Line
      ("#define pos    " & C_String (To_String (Step.R.Position)));
      ...
      S.C.Put_Line
      ("XReq_Format_Put_Step  (format, prefix, stanza, pos, " &
      ...
      if Fake then
         ...
         S.C.Put_Line
         ("XReq_Error_Make (err, ""The step definition cound " &
          "not be found"", """", 0);");
         S.C.Put_Line ("if (1) {");
      else
      ...
      if not Fake then
         ...
         if Outline then
            S.C.Put_Line
            ("XReq_Format_Put_Scenario_Outline (format" &
             ", " &
         ...
         S.C.Put_Line
         ("XReq_Format_Put_Step  (format, prefix, stanza, " &
...
--      use String_Vectors;
   ...
   begin
      ...
      if Background then
         ...
         S.C.Put_Line
         ("    XReq_Conditional_Eval_Position (cond, " &
         ...
         S.C.Put_Line ("if (!first) {");
         S.C.Put_Line
         ("  /* Background has already been shown, show " & "scenario */");
         S.C.Put_Indent;
         S.C.Put ("  XReq_Format_");
         if Scenario.R.Outline then
            S.C.Put ("Put_Outline");
         else
         ...
         S.C.Put_Line ("*****************/");
         S.C.Put_Line
         (S.Fn_Backgnd & " (format, report, is_first, cond, &fail, 0);");
      ...
      if not Background or else Scenario.R.Step_Count /= 0 then
         ...
         if Background then
            S.C.Put_Line
            ("XReq_Format_Put_Background (format, " &
             C_String (Scenario.R.Name) &
             ", " &
             C_String (To_String (Scenario.R.Position)) &
             ", " &
            ...
            S.C.Put_Line
            ("XReq_Format_Put_Outline (format, " &
             C_String (Scenario.R.Name) &
             ", " &
             C_String (To_String (Scenario.R.Position)) &
             ", " &
            ...
            S.C.Put_Line
            ("XReq_Format_Put_Scenario (format, " &
             C_String (Scenario.R.Name) &
             ", " &
             C_String (To_String (Scenario.R.Position)) &
             ", " &
      ...
      if Scenario.R.Outline then
         ...
         for J in Scenario.R.Outline_First .. Scenario.R.Outline_Last loop
            ...
            S.C.Put_Line ("XReq_Format_Start_Scenario (format);");
            for I in
              Scenario.R.Outline_Step_First (J) ..
                  Scenario.R.Outline_Step_Last (J)
            loop
               Generate_Step
                 (S,
                  Scenario,
                  Scenario.R.Outline_Step_Element (J, I),
                  M,
                  Background,
                  False,
                  True,
         ...
         S.C.Put_Line ("*******************************/");
         S.C.Put_Line
         ("XReq_Format_Put_Outline_Report " & "(format, outline_table);");
      end if;
      S.C.Put_Line ("/*******************");
      ...
      if Background then
         ...
         S.C.Put_Line
         ("  XReq_Report_num_steps_inc (report, " &
         ...
         if Scenario.R.Outline then
            for J in Scenario.R.Outline_First .. Scenario.R.Outline_Last loop
               S.C.Put_Line
               (S.Fn_Backgnd & " (format, report, is_first, cond, &fail, 1);");
            end loop;
         else
            S.C.Put_Line
            (S.Fn_Backgnd & " (format, report, is_first, cond, &fail, 1);");
         ...
         S.C.Put_Line
         ("XReq_Report_num_steps_inc (report," & Steps_Count'Img & ");");
         S.C.UnIndent (2);
         S.C.Put_Line ("}");
         S.C.UnIndent (2);
         S.C.Put_Line ("}");
   ...
   begin
      Generate_Scenario
        (S,
         S.Feature.R.Background,
         S.Fn_Backgnd,
         0,
         Num_Steps_Background,
         True);
      for I in S.Feature.R.Scenario_First .. S.Feature.R.Scenario_Last loop
         ...
         Generate_Scenario
           (S,
            S.Feature.R.all.Scenario_Element (I),
            Str,
            N,
            Num_Steps_Scenario);
         N           := N + 1;
      ...
      while Has_Element (J) loop
         Append
           (Buf,
            "#include """ &
            Goto_Path
              (Containing_Directory (To_String (S.H_File)),
               To_String (Element (J))) &
            """" &
            S.C.CRLF);
         Next (J);
   ...
   procedure Generate (Gen : in out C_Generator_Type; Log : Logger_Ptr) is
      use String_Vectors;
   ...
   begin
      ...
      Gen.C.Put_Line
      ("#define STR_Background " & C_String (Lang.R.Background));
      Gen.C.Put_Line ("#define STR_Scenario   " & C_String (Lang.R.Scenario));
      Gen.C.Put_Line
      ("#define STR_Outline    " & C_String (Lang.R.Scenario_Outline));
      ...
      Gen.C.Indent (2);
      Gen.C.Put_Line
      ("XReq_Format_Put_Feature (format, " &
      ...
      Gen.H.Put_Line
      ("#define " & Gen.Header_Name & "_num_steps" & Total_Steps'Img);
      Gen.H.New_Line;
      ...
      Gen.C.Indent (2);
      Gen.C.Put_Line
      ("XReq_Format_List_Feature (format, " &
       C_String (Gen.Feature.R.Name) &
       ");");
      ...
      for I in Gen.Feature.R.Scenario_First .. Gen.Feature.R.Scenario_Last loop
         ...
         Gen.C.Put_Line
         ("XReq_Format_List_Scenario (format, " &
          C_String (E.R.Name) &
          ", " &
   ...
   function Full_Name (Gen : C_Generator_Type) return String is
   begin
...
--      use Ada.Directories;
   ...
   begin
      ...
      while Has_Element (I) loop
         ...
         Body_B.Put_Line
         ("run_" & E.Full_Name & " (format, cond, report, 0, 1);");
         Next (I);
      end loop;
      Body_B.Put_Line
      ("XReq_Format_Set_Num_Steps (format, " &
      ...
      Body_B.Indent (2);
      Body_B.Put_Line
      ("XReq_Format_Put_Summary " & "(format, report, time_delta);");
      ...
      if Make then
         ...
         declare
            ...
            Args : constant Argument_List (1 .. 5) :=
              (Arg1'Unchecked_Access,
               Arg2'Unchecked_Access,
               Arg3'Unchecked_Access,
               Arg4'Unchecked_Access,
