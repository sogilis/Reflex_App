...
package body XReq.Job is
   ...
   procedure Make (Job : out Job_Type; Feature_File : String) is
   begin
      Job :=
   ...
   function Feature_File (Job : Job_Type) return String is
   begin
   ...
   function Result (Job : Job_Type) return Result_Feature_Handle is
   begin
      return Job.Result;
   end Result;
   ...
   procedure Run
     (Job               : in out Job_Type;
      ...
      Logger            :        Logger_Ptr;
      Add_Steps_Pkg     :        String  := "";
      Step_Matching     :        Boolean := False;
      Syntax_Check_Only :        Boolean := False)
   is
      use String_Sets;
   ...
   begin
      ...
      if not Syntax_Check_Only then
         ...
         if Add_Steps_Pkg /= "" and not Is_Empty (Missing_Steps) then
            Env.Ref.Steps.Ref.Add_Steps (Missing_Steps,
                                         Add_Steps_Pkg,
                                         Env.Ref.First_Step_Dir,
                                         Env.Ref.Language,
                                         Logger);
            Clear (Missing_Steps);
         ...
         --  Create missing set_up and tear_down step definitions
         if Add_Steps_Pkg /= "" then
            Env.Ref.Steps.Ref.Add_Set_Up_And_Tear_Down (Add_Steps_Pkg,
                                                        Env.Ref.First_Step_Dir,
                                                        Env.Ref.Language,
                                                        Logger);
         end if;
