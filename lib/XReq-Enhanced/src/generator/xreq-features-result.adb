...
package body XReq.Features.Result is
   ...
   procedure Process_Feature
     (Res           : in out Result_Feature_Type;
   ...
   begin
      ...
      (Feature.R.Background, Steps, Log, Errors, Missing_Steps, Step_Matching);
      ...
      for I in Feature.R.Scenario_First .. Feature.R.Scenario_Last loop
         ...
         R_Scen  := Create;
         R_Scen.Ref.Process_Scenario
         (Feature.R.Scenario_Element
          (I), Steps, Log, Errors, Missing_Steps, Step_Matching);
         if Errors then
            Res.Fail := True;
         else
      ...
      if Res.Fail then
         Log.Put_Line
         ("XReq can create the procedures for you if you " &
          "use --fill-steps");
      end if;
      pragma Assert (Res.Language.Valid);
      pragma Assert (Res.Background.Valid);
      pragma Assert
        (Res.Fail or Res.Scenario_Count = Feature.R.Scenario_Count);
   end Process_Feature;
   ...
   function Fail (F : Result_Feature_Type) return Boolean is
   begin
      return F.Fail;
   end Fail;
   ...
   function Background
     (F : Result_Feature_Type) return Result_Scenario_Handle
   is
   ...
   function Scenario_Element
     (F : Result_Feature_Type;
      I : Natural) return Result_Scenario_Handle
   is
      Super : constant access constant Feature_Type'Class := F'Access;
