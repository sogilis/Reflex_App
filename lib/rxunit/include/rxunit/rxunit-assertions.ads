...
package Rxunit.Assertions is
   ...
   type Test is abstract new Rxunit.Tests.Test with private;
   ...
   function Get_Failure
     (I : Failure_Iter) return Rxunit.Test_Results.Test_Failure;
...
private
   ...
   package Failure_Lists is
      new Ada.Containers.Doubly_Linked_Lists (Failure_Elt);
   --  Container for failed assertion messages per routine

   type Failure_Iter is new Failure_Lists.Cursor;

   type Test is abstract new Rxunit.Tests.Test with record
      Id : Test_Id := Null_Id;
   end record;
